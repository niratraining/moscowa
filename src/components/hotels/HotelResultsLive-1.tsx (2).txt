...
export function HotelResultsLive({
  ...
  // "recommended" keeps the API's own order (already cheapest-first from
  // the D1 query); "price" sorts explicitly so the pill visibly does something
  // even if the two happen to match today.
  ...
  return (
    <ResultsShell
      resultCount={sortedHotels?.length ?? 0}
      hideResultCount
      sortOptions={[
    ...
    >
      ...
      {!loading && sortedHotels && sortedHotels.length === 0 && !error && (
        <p className="rounded-xl bg-moscowa-bg-secondary p-3 text-[13px] text-moscowa-text-secondary">
